...
const PAGE_SIZE = 24;
const TYPE_OPTIONS = ['All', 'Preset', 'Sound Effect', 'Background', 'PNG'];
const SORT_OPTIONS = ['Newest', 'Most Downloaded'];

const downloadAsset = async (id, filename, showToast) => {
  try {
    const response = await fetch(`/api/download/editing?id=${id}`);
...
export default function EditingAssets() {
    ...
    <Layout title="CCAVE • Creator Coaster Asset Vault Enterprise">
      <section className="hero">
        <div>
          ...
          <p>Search presets, sound effects, backgrounds, PNGs...</p>
        </div>
      </section>
      ...
      <div className="search-row">
        <input
          className="input"
          placeholder="Search presets, sound effects, backgrounds, PNGs..."
          value={query}
      ...
      {!loading && assets.length === 0 && (
        <div className="footer-note">No editing assets found.</div>
      )}
